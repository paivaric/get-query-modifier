...
function getQueryModifier(query, options) {
  ...
  return function queryModifier(query) {
    ...
    if(operators.$deepPopulate) {
      if(Array.isArray(operators.$deepPopulate)) {
        operators.$deepPopulate.forEach(function (p) {
          query = query.deepPopulate(p);
        });
      } else {
        query = query.deepPopulate(operators.$deepPopulate);
      }
